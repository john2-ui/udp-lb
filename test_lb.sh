...
if ! ip netns list | grep -q "ns-lb"; then
    echo "⚠️ 检测到网络环境未配置，正在自动执行 setup_env.sh..."
...
do
    ...
    if [[ "$RESPONSE" == *"Real Server 1"* ]]; then
        echo "第 $i 次请求 🎯 命中 -> [RS1] 192.168.1.10"
        ((RS1_COUNT++))
    elif [[ "$RESPONSE" == *"Real Server 2"* ]]; then
        echo "第 $i 次请求 🎯 命中 -> [RS2] 192.168.1.11"
        ((RS2_COUNT++))
    else
        echo "第 $i 次请求 ❌ 失败 -> 收到未知响应或超时: '$RESPONSE'"
    fi
    sleep 0.2
done
...
if [ $RS1_COUNT -gt 0 ] && [ $RS2_COUNT -gt 0 ]; then
    echo -e "\n🎉 恭喜！测试【通过】: 成功验证了跨 Namespace 的 FullNAT 转发和动态一致性哈希分流！"
else
